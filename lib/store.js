...
const EMPTY = { users: [], requests: [], claims: [], messages: [], reviews: [], projobs: [], feedback: [], settings: {}, meta: { invoiceYear: 0, invoiceSeq: 0 } };

let db;
function load() {
  try { db = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8')); }
  catch { db = JSON.parse(JSON.stringify(EMPTY)); }
  for (const k of ['users', 'requests', 'claims', 'messages', 'reviews', 'projobs', 'feedback']) if (!Array.isArray(db[k])) db[k] = [];
...
module.exports = {
  ...
  updateRequest: (rid, patch) => { const r = db.requests.find(x => x.id === rid); if (r) { Object.assign(r, patch); save(); } return r; },
  openRequests: () => db.requests.filter(r => r.status === 'open').sort((a, b) => b.createdAt - a.createdAt),
  listRequests: () => db.requests.slice().sort((a, b) => b.createdAt - a.createdAt),
  ...
  updateClaim: (cid, patch) => { const c = db.claims.find(x => x.id === cid); if (c) { Object.assign(c, patch); save(); } return c; },
  listClaims: () => db.claims.slice(),
  // ---- messages (chat) ----
  ...
  // feedback / ideeën
  addFeedback: (f) => { f.id = id(); f.createdAt = Date.now(); f.votes = []; f.status = 'nieuw'; db.feedback.push(f); save(); return f; },
  listFeedback: () => db.feedback.slice().sort((a, b) => (b.votes.length - a.votes.length) || (b.createdAt - a.createdAt)),
  findFeedback: (fid) => db.feedback.find(f => f.id === fid),
  updateFeedback: (fid, patch) => { const f = db.feedback.find(x => x.id === fid); if (f) { Object.assign(f, patch); save(); } return f; },
  // instellingen (beheerpaneel: prijzen e.d.)
  getSettings: () => db.settings || {},
  updateSettings: (patch) => { db.settings = Object.assign(db.settings || {}, patch); save(); return db.settings; },
