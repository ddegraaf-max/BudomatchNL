...
module.exports = {
  ...
  async listUsers() {
    const r = await q('SELECT data FROM users');
    return r.rows.map(x => x.data);
  },
  async addUser(u) {
  ...
  async listRequests() {
    const r = await q('SELECT data FROM requests ORDER BY created_at DESC');
    return r.rows.map(x => x.data);
  },
  ...
  async updateRequest(rid, patch) {
    const r = await this.findRequest(rid);
    if (!r) return undefined;
    Object.assign(r, patch);
    await q('UPDATE requests SET status=$2, data=$3 WHERE id=$1', [rid, r.status || 'open', JSON.stringify(r)]);
    return r;
  },
  // ---- claims ----
  ...
  async listClaims() {
    const r = await q('SELECT data FROM claims');
    return r.rows.map(x => x.data);
  },
  async updateClaim(cid, patch) {
    const r = await q('SELECT data FROM claims WHERE id=$1 LIMIT 1', [cid]);
    if (!r.rows[0]) return undefined;
    const c = Object.assign(r.rows[0].data, patch);
    await q('UPDATE claims SET data=$2 WHERE id=$1', [cid, JSON.stringify(c)]);
    return c;
  },
  // ---- messages (chat) ----
  async addMessage(m) {
  ...
  async listReviews() {
    const r = await q('SELECT data FROM reviews');
    return r.rows.map(x => x.data);
  },
  ...
  // ---- feedback / ideeën ----
  async addFeedback(f) {
    f.id = id(); f.createdAt = Date.now(); f.votes = []; f.status = 'nieuw';
    await q('INSERT INTO feedback (id, data, created_at) VALUES ($1,$2,$3)', [f.id, JSON.stringify(f), f.createdAt]);
    return f;
  },
  async listFeedback() {
    const r = await q('SELECT data FROM feedback');
    return r.rows.map(x => x.data).sort((a, b) => ((b.votes || []).length - (a.votes || []).length) || (b.createdAt - a.createdAt));
  },
  async findFeedback(fid) {
    const r = await q('SELECT data FROM feedback WHERE id=$1 LIMIT 1', [fid]);
    return r.rows[0] ? r.rows[0].data : undefined;
  },
  async updateFeedback(fid, patch) {
    const f = await this.findFeedback(fid);
    if (!f) return undefined;
    Object.assign(f, patch);
    await q('UPDATE feedback SET data=$2 WHERE id=$1', [fid, JSON.stringify(f)]);
    return f;
  },
  // ---- instellingen (beheerpaneel: prijzen e.d.) ----
  async getSettings() {
    const r = await q("SELECT value FROM settings WHERE key='app' LIMIT 1");
    return r.rows[0] ? r.rows[0].value : {};
  },
  async updateSettings(patch) {
    const next = Object.assign(await this.getSettings(), patch);
    await q("INSERT INTO settings (key, value) VALUES ('app', $1) ON CONFLICT (key) DO UPDATE SET value = $1", [JSON.stringify(next)]);
    return next;
  },
